<x-app-layout>
    <div class="py-6">
        <div class="container">
            <div class="p-2">
                <h1 class="text-4xl font-bold text-gray-700 mb-10">{{$post->name}}</h1>
                <div class="text-lg text-gray-700 my-4 break-words font-semibold">
                    {!!$post->extract!!}
                </div>
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <div id="post-body" class="my-6 text-base text-gray-700 break-words font-semibold">
                            {!! $post->body !!}
                        </div>
                    </div>
                    {{-- Related content --}}
                            ...
                            @foreach ($similars as $similar)
                                <li class="mb-4">
                                    <a class="flex mt-2" href="{{route('posts.show', $similar)}}">
                                        @if ($similar->image)
                                            <img class="rounded-2xl w-30 h-20 object-cover object-center" src="{{Storage::url($similar->image->url)}}" alt="">
                                        @else
